...
function install_ssl_Cert() {
    if [ -d "/opt/letsencrypt/" ]
        ...
        pip3 install certbot-dns-namecheap --break-system-packages > /dev/null 2>&1
    ...
    case ${answer:0:1} in
        y|Y )
            ...
            case ${answer:0:1} in
                y|Y )
                    ...
                    if [ "$apikeyValue" = "<APIKEY>" ] || [ "$usernameValue" = "<USERNAME>" ]; then
                        echo $'\n[!]\tAPI credentials not configured in script.'
                        read -p "Enter your Namecheap username: " -r usernameValue
                        read -p "Enter your Namecheap API key: " -r apikeyValue
                        
                        # Update the variables in the script file for future use
                        sed -i "s/apikeyValue=\"<APIKEY>\"/apikeyValue=\"$apikeyValue\"/" "$0"
                        sed -i "s/usernameValue=\"<USERNAME>\"/usernameValue=\"$usernameValue\"/" "$0"
                    fi
                    
                    # Get client IP if not already set
                    if [ -z "$updateIP" ] || [ "$updateIP" = "" ]; then
                        updateIP=$(curl -s icanhazip.com)
                    fi
                    
                    # Create the credentials file directory if it doesn't exist
                    mkdir -p /etc/letsencrypt
                    
                    # Create the Namecheap credentials file with correct format (no spaces around =)
                    echo "# Namecheap API credentials used by Certbot" > /etc/letsencrypt/namecheap.ini
                    echo "dns_namecheap_username=$usernameValue" >> /etc/letsencrypt/namecheap.ini
                    echo "dns_namecheap_api_key=$apikeyValue" >> /etc/letsencrypt/namecheap.ini
                    
                    # Secure the credentials file
                    chmod 600 /etc/letsencrypt/namecheap.ini
                    
                    echo $'\n[+]\tNamecheap credentials file created at /etc/letsencrypt/namecheap.ini'
                    echo $'[+]\tAttempting to obtain certificate using Namecheap DNS API...\n'
                    
                    # Extract domain parts for API calls
                    IFS="." read -ra values <<< "$domain"
                    sld=${values[0]}
                    tld=${values[1]}
                    
                    # Pre-clean any existing _acme-challenge records to avoid conflicts
                    echo $'[+]\tCleaning up any existing _acme-challenge records...\n'
                    curl -s "https://api.namecheap.com/xml.response?ApiUser=${usernameValue}&ApiKey=${apikeyValue}&UserName=${usernameValue}&Command=namecheap.domains.dns.getHosts&ClientIp=${updateIP}&SLD=${sld}&TLD=${tld}" > /tmp/current_records.xml
                    
                    # Run certbot with increased propagation time
                    certbot certonly \
                        -a dns-namecheap \
                    ...
                    if [ $? -eq 0 ]; then
                        ...
                        echo $'\n[-]\tFailed to obtain certificate using Namecheap DNS API.'
                        echo $'[-]\tCheck your Namecheap API credentials and ensure your account has API access enabled.\n'
                        
                        # Verify DNS records manually
                        echo $'[+]\tAttempting to verify DNS records manually...\n'
                        dig _acme-challenge.${domain} TXT
                        
                        # Fallback to manual method if API fails
                        echo $'[!]\tFalling back to manual DNS challenge method...\n'
                        
                        echo $'\nUse this reference API call to enter the upcoming certbot ACME challenges:'
                        echo "curl \"https://api.namecheap.com/xml.response?ApiUser=${usernameValue}&ApiKey=${apikeyValue}&UserName=${usernameValue}&Command=namecheap.domains.dns.setHosts&ClientIp=${updateIP}&SLD=${sld}&TLD=${tld}&HostName1=_acme-challenge&RecordType1=TXT&Address1=<CERTBOT-TXT-1>&TTL1=300&HostName2=_acme-challenge&RecordType2=TXT&Address2=<CERTBOT-TXT-2>&TTL2=300\""
                        ...
                        command="certbot certonly --manual --register-unsafely-without-email --agree-tos --preferred-challenges dns -d '${domain}' -d '*.${domain}'"
                        eval $command
                    fi
                ;;
                * )
                    # Manual DNS challenge method for non-Namecheap users
        ...
        ;;
        * )
            # Standard certificate process (unchanged)

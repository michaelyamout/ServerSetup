...
function install_ssl_Cert() {
    echo $'\n[ ] Setting up SSL certificates using acme.sh with Namecheap DNS validation'
    
    # Ensure we have git installed
    apt-get install -qq -y git curl > /dev/null 2>&1
    
    # Use the global variables if they exist, otherwise prompt for them
    if [ -n "$usernameValue" ] && [ -n "$apikeyValue" ] && [ -n "$updateIP" ]; then
        echo "[ ] Using Namecheap credentials from script configuration"
        namecheap_username="$usernameValue"
        namecheap_apikey="$apikeyValue"
        
        # Set up environment variables for acme.sh
        export NAMECHEAP_USERNAME="${namecheap_username}"
        export NAMECHEAP_API_KEY="${namecheap_apikey}"
        export NAMECHEAP_SOURCEIP="${updateIP}"
        
        echo $'\n[ ] Your current IP address is: '"${updateIP}"
        echo $'[ ] Make sure this IP is whitelisted in your Namecheap API settings\n'
    else
        echo "[ ] Namecheap credentials not found in script configuration"
        
        # Get the current external IP if not already defined
        if [ -z "$updateIP" ]; then
            updateIP=$(curl -s icanhazip.com)
        fi
        
        # Prompt for Namecheap credentials
        read -p "Enter your Namecheap username: " namecheap_username
        read -p "Enter your Namecheap API key: " namecheap_apikey
    ...
    # Clone acme.sh repository
    if [ ! -d "/opt/acme.sh" ]; then
        echo "[ ] Cloning acme.sh repository..."
        git clone https://github.com/acmesh-official/acme.sh.git /opt/acme.sh > /dev/null 2>&1
    else
        echo "[ ] acme.sh repository already exists, updating..."
        cd /opt/acme.sh
        git pull > /dev/null 2>&1
    fi
    
    # Prompt for domain information
    echo $'\n[ ] Please enter domain information for certificate issuance'
    read -p "Enter your primary domain (e.g., example.com): " primary_domain
    read -p "Do you want to include a wildcard certificate (*.${primary_domain})? (y/N): " include_wildcard
    
    # Prepare domain parameters
    domain_params="-d ${primary_domain}"
    if [[ "${include_wildcard}" =~ ^[Yy] ]]; then
        domain_params="${domain_params} -d *.${primary_domain}"
        echo "[ ] Including wildcard certificate for *.${primary_domain}"
    fi
    
    # Create the target directory if it doesn't exist
    mkdir -p /etc/letsencrypt/live/${primary_domain}
    
    # Issue the certificate with --force to allow sudo
    echo $'\n[ ] Issuing certificate using acme.sh with Namecheap DNS validation'
    cd /opt/acme.sh
    
    # First, make sure there's at least one DNS record for the domain
    # This is to avoid the "Hosts not found" error we saw in the debug logs
    echo "[ ] Checking for existing DNS records..."
    
    # Extract domain components for Namecheap API
    IFS='.' read -ra DOMAIN_PARTS <<< "$primary_domain"
    DOMAIN_LENGTH=${#DOMAIN_PARTS[@]}
    
    if [ "$DOMAIN_LENGTH" -eq 2 ]; then
        # For example.com format
        SLD="${DOMAIN_PARTS[0]}"
        TLD="${DOMAIN_PARTS[1]}"
    elif [ "$DOMAIN_LENGTH" -gt 2 ]; then
        # For sub.example.com format - get the main domain
        SLD="${DOMAIN_PARTS[DOMAIN_LENGTH-2]}"
        TLD="${DOMAIN_PARTS[DOMAIN_LENGTH-1]}"
    else
    ...
    dns_check=$(curl -s "https://api.namecheap.com/xml.response?ApiUser=${NAMECHEAP_USERNAME}&ApiKey=${NAMECHEAP_API_KEY}&UserName=${NAMECHEAP_USERNAME}&Command=namecheap.domains.dns.getHosts&ClientIp=${NAMECHEAP_SOURCEIP}&SLD=${SLD}&TLD=${TLD}")
    
    # If no hosts found, add a basic A record
    if echo "$dns_check" | grep -q "IsUsingOurDNS=\"true\"" && ! echo "$dns_check" | grep -q "<host"; then
        echo "[ ] No DNS records found. Adding a basic A record to enable API functionality..."
        curl -s "https://api.namecheap.com/xml.response?ApiUser=${NAMECHEAP_USERNAME}&ApiKey=${NAMECHEAP_API_KEY}&UserName=${NAMECHEAP_USERNAME}&Command=namecheap.domains.dns.setHosts&ClientIp=${NAMECHEAP_SOURCEIP}&SLD=${SLD}&TLD=${TLD}&HostName1=@&RecordType1=A&Address1=${NAMECHEAP_SOURCEIP}&TTL1=300" > /dev/null
        echo "[ ] Basic A record added"
    fi
    
    # Explicitly specify Let's Encrypt as the CA and use --force for sudo
    ./acme.sh --issue --dns dns_namecheap ${domain_params} --server letsencrypt --force
    
    # Check if certificate was issued successfully
    if [ ! -f "$HOME/.acme.sh/${primary_domain}_ecc/${primary_domain}.cer" ]; then
        echo $'\n[ - ] Certificate issuance failed!'
        echo "[ ] Try running with debug: ./acme.sh --issue --dns dns_namecheap ${domain_params} --server letsencrypt --debug --force"
        return 1
    fi
    
    # Install the certificate to the standard location
    echo $'\n[ ] Installing certificate to /etc/letsencrypt/live/'"${primary_domain}"
    ./acme.sh --install-cert -d ${primary_domain} --ecc \
      --cert-file /etc/letsencrypt/live/${primary_domain}/cert.pem \
      --key-file /etc/letsencrypt/live/${primary_domain}/privkey.pem \
      --fullchain-file /etc/letsencrypt/live/${primary_domain}/fullchain.pem \
      --force
    
    # Create chain.pem manually if needed
    if [ -f "$HOME/.acme.sh/${primary_domain}_ecc/ca.cer" ]; then
        cp "$HOME/.acme.sh/${primary_domain}_ecc/ca.cer" "/etc/letsencrypt/live/${primary_domain}/chain.pem"
        echo "[ ] Manually created chain.pem file"
    fi
    
    # Verify certificate installation
    if [ -f "/etc/letsencrypt/live/${primary_domain}/fullchain.pem" ]; then
        echo $'\n[ + ] Certificate successfully installed!'
        echo "[ + ] Certificate location: /etc/letsencrypt/live/${primary_domain}/"
        echo "[ + ] Auto-renewal has been configured"
        
        # Display certificate information
        echo $'\n[ ] Certificate information:'
        openssl x509 -in /etc/letsencrypt/live/${primary_domain}/cert.pem -noout -text | grep -E 'Subject:|Issuer:|Not Before:|Not After :|DNS:'
        
        # Set proper permissions
        chmod -R 755 /etc/letsencrypt/live
        chmod 644 /etc/letsencrypt/live/${primary_domain}/*.pem
    else
